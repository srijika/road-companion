...
module.exports = {
    ...
    getNearByTrips: async (req, res, next) => {
        try {
            let trips = await UserTrip.find({}).populate('user_id').lean().exec();
            // let user_vehicle = await UserVehicle.findOne({user_id: user_id});
            // console.log(user_vehicle);
            
            
            return res.status(200).send({ status: 200, data: trips, message: "trips get successfully" });
        } catch (e) {
            console.log(e);
            return res.status(400).send({ status: 400, message: e.message });
        }
    ...
    getTripDetail: async (req, res, next) => {
        try {   
            let trip_id = req.body.trip_id;
            let trip = await UserTrip.findById(trip_id).lean().exec();

            console.log('trip');
            console.log(trip);

            return res.status(200).send({ status: 200, data: trip, message: "trip detail get successfully" });
        } catch (e) {
            console.log(e);
            return res.status(400).send({ status: 400, message: e.message });
        }
